...
begin
  ...
  if @PartsSource = 'Logistics Database' or @PartsSource = '''Logistics Database''' then
    select "STRING"('VesselId','=',
      ...
      @OrderBy as "OrderBy",
      "LogisticsHSC" as "HSC"
      from "ACM"."PartsSearchLogisticsParts"(@VesselId,@EquipmentId,@UseVessel,@HSC,@APL,@CAGE,@Manufacturer,@ModelNumber,@NavyStockNumber,@PartNumber,@PartDescription,@SupplierName,@CageCode)
  end if
exception
  when others then
